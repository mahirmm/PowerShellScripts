...
foreach ($slot in $timeSlots) { # stores each field in PS variables
    ...
    if ($session -eq "Break" -or [string]::IsNullOrWhiteSpace($session)) {
        ...
        # replace ":" with "-" in the task name as is illegal char in TaskScheduler task names
        ...
        $addToCollTaskName = "$taskNamePrefix-$weekPosition-$day-$safeStartTime" # title for task name eg. 'CM142-01-Monday-00-00'
        ...
        $removePXETaskName = "$taskNamePrefix-RemoveCollection-$weekPosition-$day-$safeStartTime" # title for task name eg. 'CM142-RemoveCollection-01-Monday-00-00'
